...
class ShopUsecases {
  ...
  Future<ShopItem> fetchShopItemById(String id) => _repository.fetchShopItemById(id);
  Future<ShopCartSnapshot> fetchCartItems() => _repository.fetchCartItems();
  Future<void> saveCartItems(Map<String, int> items) => _repository.saveCartItems(items);
  Future<String> placeOrder({required String shippingAddress, required String customerName, required String customerEmail, required String phone, required String city, required String postalCode, required String paymentMethod}) =>
      _repository.placeOrder(shippingAddress: shippingAddress, customerName: customerName, customerEmail: customerEmail, phone: phone, city: city, postalCode: postalCode, paymentMethod: paymentMethod);
  Future<String> getEsewaPaymentUrl({required double amount, required String orderId, required String productCode}) =>
      _repository.getEsewaPaymentUrl(amount: amount, orderId: orderId, productCode: productCode);
  Future<bool> verifyEsewaPayment({required double amount, required String orderId, required String productCode}) =>
      _repository.verifyEsewaPayment(amount: amount, orderId: orderId, productCode: productCode);
